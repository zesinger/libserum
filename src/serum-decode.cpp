...
UINT16 lastfrx[MAX_SPRITE_TO_DETECT],
    lastfry[MAX_SPRITE_TO_DETECT];  // last position in the frame of the sprite
UINT16 lastspx[MAX_SPRITE_TO_DETECT],
    lastspy[MAX_SPRITE_TO_DETECT];  // last top left of the sprite to display
UINT16 lastwid[MAX_SPRITE_TO_DETECT],
    lasthei[MAX_SPRITE_TO_DETECT];  // last dimensions of the sprite to display
...
UINT32 crc32_fast(
    UINT8* s, UINT n,
    UINT8 ShapeMode)  // computing a buffer CRC32, "CRC32encode()" must have
                      // been called before the first use
{
...
UINT32 crc32_fast_mask(
    UINT8* source, UINT8* mask, UINT n,
    UINT8 ShapeMode)  // computing a buffer CRC32 on the non-masked area,
                      // "CRC32encode()" must have been called before the first
                      // use
...
void Reset_ColorRotations(void) {
  ...
  colorshiftinittime[0] =
      std::chrono::duration_cast<std::chrono::milliseconds>(
          std::chrono::system_clock::now().time_since_epoch())
          .count();
  for (int ti = 1; ti < MAX_COLOR_ROTATIONS; ti++)
    colorshiftinittime[ti] = colorshiftinittime[0];
}

SERUM_API bool Serum_LoadFile(const char* const filename, int* pwidth,
                              int* pheight, unsigned int* pnocolors,
  ...
  framespriteBB =
      (UINT16*)malloc(nframes * MAX_SPRITES_PER_FRAME * 4 * sizeof(UINT16));
  spritedescriptionso =
      (UINT8*)malloc(nsprites * MAX_SPRITE_SIZE * MAX_SPRITE_SIZE);
  spritedescriptionsc =
      (UINT8*)malloc(nsprites * MAX_SPRITE_SIZE * MAX_SPRITE_SIZE);
  activeframes = (UINT8*)malloc(nframes);
  colorrotations = (UINT8*)malloc(nframes * 3 * MAX_COLOR_ROTATIONS);
  spritedetdwords =
      (UINT32*)malloc(nsprites * sizeof(UINT32) * MAX_SPRITE_DETECT_AREAS);
  spritedetdwordpos =
      (UINT16*)malloc(nsprites * sizeof(UINT16) * MAX_SPRITE_DETECT_AREAS);
  spritedetareas =
      (UINT16*)malloc(nsprites * sizeof(UINT16) * MAX_SPRITE_DETECT_AREAS * 4);
  ...
  for (int ti = 0; ti < (int)nsprites * MAX_SPRITE_SIZE * MAX_SPRITE_SIZE;
       ti++) {
  ...
  fread(spritedetdwords, sizeof(UINT32), nsprites * MAX_SPRITE_DETECT_AREAS,
        pfile);
  fread(spritedetdwordpos, sizeof(UINT16), nsprites * MAX_SPRITE_DETECT_AREAS,
        pfile);
  fread(spritedetareas, sizeof(UINT16), nsprites * 4 * MAX_SPRITE_DETECT_AREAS,
        pfile);
  if (sizeheader >= 11 * sizeof(UINT))
    fread(triggerIDs, sizeof(UINT32), nframes, pfile);
  else
    memset(triggerIDs, 0xFF, sizeof(UINT32) * nframes);
  if (sizeheader >= 12 * sizeof(UINT))
    fread(framespriteBB, sizeof(UINT16), nframes * MAX_SPRITES_PER_FRAME * 4,
          pfile);
  else {
  ...
  if (sizeheader >= 13 * sizeof(UINT)) {
    ...
    memset(backgroundIDs, 0xFF, nframes * sizeof(UINT16));
  fclose(pfile);
  if (pntriggers) {
    *pntriggers = 0;
...
SERUM_API bool Serum_Load(const char* const altcolorpath,
                          const char* const romname, int* pwidth, int* pheight,
                          unsigned int* pnocolors, unsigned int* pntriggers) {
  char pathbuf[pathbuflen];
...
int32_t Identify_Frame(UINT8* frame) {
  ...
  // with 0. So we ned a helper to be able to detect frame 0 as new.
  ...
  do {
    if (!framechecked[tj]) {
      ...
      if (mask < 255) {
        ...
        Hashc = crc32_fast(frame, pixels, Shape);
...
bool Check_Sprites(UINT8* Frame, int quelleframe, UINT8* pquelsprites,
                   UINT8* nspr, UINT16* pfrx, UINT16* pfry, UINT16* pspx,
                   UINT16* pspy, UINT16* pwid, UINT16* phei) {
  UINT8 ti = 0;
  UINT32 mdword;
  *nspr = 0;
  while ((ti < MAX_SPRITES_PER_FRAME) &&
         (framesprites[quelleframe * MAX_SPRITES_PER_FRAME + ti] < 255)) {
    UINT8 qspr = framesprites[quelleframe * MAX_SPRITES_PER_FRAME + ti];
    short minxBB =
        (short)framespriteBB[(quelleframe * MAX_SPRITES_PER_FRAME + ti) * 4];
    short minyBB = (short)
        framespriteBB[(quelleframe * MAX_SPRITES_PER_FRAME + ti) * 4 + 1];
    short maxxBB = (short)
        framespriteBB[(quelleframe * MAX_SPRITES_PER_FRAME + ti) * 4 + 2];
    short maxyBB = (short)
        framespriteBB[(quelleframe * MAX_SPRITES_PER_FRAME + ti) * 4 + 3];
    for (UINT32 tm = 0; tm < MAX_SPRITE_DETECT_AREAS; tm++) {
      if (spritedetareas[qspr * MAX_SPRITE_DETECT_AREAS * 4 + tm * 4] == 0xffff)
        continue;
      // we look for the sprite in the frame sent
      for (short ty = minyBB; ty <= maxyBB; ty++) {
        mdword = (UINT32)(Frame[ty * fwidth + minxBB] << 8) |
                 (UINT32)(Frame[ty * fwidth + minxBB + 1] << 16) |
        ...
        for (short tx = minxBB; tx <= maxxBB - 3; tx++) {
          ...
          if (mdword == spritedetdwords[qspr * MAX_SPRITE_DETECT_AREAS + tm]) {
            ...
            short detx = (short)
                spritedetareas[qspr * MAX_SPRITE_DETECT_AREAS * 4 + tm * 4];
            short dety = (short)
                spritedetareas[qspr * MAX_SPRITE_DETECT_AREAS * 4 + tm * 4 + 1];
            short detw = (short)
                spritedetareas[qspr * MAX_SPRITE_DETECT_AREAS * 4 + tm * 4 + 2];
            short deth = (short)
                spritedetareas[qspr * MAX_SPRITE_DETECT_AREAS * 4 + tm * 4 + 3];
            if ((frax - minxBB < sprx - detx) || (fray - minyBB < spry - dety))
              continue;  // if the detection area is outside the frame, continue
            int offsx = frax - sprx + detx;
            int offsy = fray - spry + dety;
            if ((offsx + detw > (int)maxxBB) || (offsy + deth > (int)maxyBB))
              continue;
            // we can now check if the sprite is there
            bool notthere = false;
            for (UINT16 tk = 0; tk < deth; tk++) {
              for (UINT16 tl = 0; tl < detw; tl++) {
                UINT8 val = spritedescriptionso[qspr * MAX_SPRITE_SIZE *
                                                    MAX_SPRITE_SIZE +
                                                (tk + dety) * MAX_SPRITE_SIZE +
                                                tl + detx];
                if (val == 255) continue;
            ...
            if (!notthere) {
              ...
              if (frax - minxBB < sprx) {
                ...
                pwid[*nspr] = MIN((UINT16)(maxxBB - minxBB + 1),
                                  (UINT16)(MAX_SPRITE_SIZE - pspx[*nspr]));
              } else {
                pspx[*nspr] = 0;
                pfrx[*nspr] = (UINT16)(frax - sprx);
                pwid[*nspr] = MIN((UINT16)(maxxBB + 1 - pfrx[*nspr]),
                                  (UINT16)(MAX_SPRITE_SIZE - pfrx[*nspr]));
              }
              if (fray - minyBB < spry) {
                pspy[*nspr] = (UINT16)(spry - (fray - minyBB));
                pfry[*nspr] = (UINT16)minyBB;
                phei[*nspr] = MIN((UINT16)(maxyBB - minyBB + 1),
                                  (UINT16)(MAX_SPRITE_SIZE - pspy[*nspr]));
              } else {
                pspy[*nspr] = 0;
                pfry[*nspr] = (UINT16)(fray - spry);
                phei[*nspr] = MIN((UINT16)(maxyBB + 1 - pfry[*nspr]),
                                  (UINT16)(MAX_SPRITE_SIZE - pfry[*nspr]));
              }
...
void Colorize_Frame(UINT8* frame, int IDfound) {
  ...
  for (tj = 0; tj < fheight; tj++) {
    for (ti = 0; ti < fwidth; ti++) {
      ...
      if ((backgroundIDs[IDfound] < nbackgrounds) && (frame[tk] == 0) &&
          (ti >= backgroundBB[IDfound * 4]) &&
          (tj >= backgroundBB[IDfound * 4 + 1]) &&
          (ti <= backgroundBB[IDfound * 4 + 2]) &&
          (tj <= backgroundBB[IDfound * 4 + 3]))
        frame[tk] =
            backgroundframes[backgroundIDs[IDfound] * fwidth * fheight + tk];
      else {
        UINT8 dynacouche = dynamasks[IDfound * fwidth * fheight + tk];
        if (dynacouche == 255)
          frame[tk] = cframes[IDfound * fwidth * fheight + tk];
        else
          frame[tk] = dyna4cols[IDfound * MAX_DYNA_4COLS_PER_FRAME * nocolors +
                                dynacouche * nocolors + frame[tk]];
      }
    }
  }
}

void Colorize_Sprite(UINT8* frame, UINT8 nosprite, UINT16 frx, UINT16 fry,
                     UINT16 spx, UINT16 spy, UINT16 wid, UINT16 hei) {
  for (UINT16 tj = 0; tj < hei; tj++) {
    for (UINT16 ti = 0; ti < wid; ti++) {
      if (spritedescriptionso[(nosprite * MAX_SPRITE_SIZE + tj + spy) *
                                  MAX_SPRITE_SIZE +
                              ti + spx] < 255) {
        frame[(fry + tj) * fwidth + frx + ti] =
            spritedescriptionsc[(nosprite * MAX_SPRITE_SIZE + tj + spy) *
                                    MAX_SPRITE_SIZE +
                                ti + spx];
      }
    }
...
SERUM_API void Serum_SetStandardPalette(const UINT8* palette,
                                        const int bitDepth) {
...
SERUM_API bool Serum_ColorizeWithMetadata(UINT8* frame, int width, int height,
                                          UINT8* palette, UINT8* rotations,
                                          UINT32* triggerID, UINT32* hashcode,
                                          int32_t* frameID) {
  if (triggerID) {
  ...
  *frameID = Identify_Frame(frame);
  ...
  UINT16 frx[MAX_SPRITE_TO_DETECT], fry[MAX_SPRITE_TO_DETECT],
      spx[MAX_SPRITE_TO_DETECT], spy[MAX_SPRITE_TO_DETECT],
      wid[MAX_SPRITE_TO_DETECT], hei[MAX_SPRITE_TO_DETECT];
  memset(nosprite, 255, MAX_SPRITE_TO_DETECT);

  if (*frameID != IDENTIFY_NO_FRAME) {
    lastframe_found = std::chrono::duration_cast<std::chrono::milliseconds>(
                          std::chrono::system_clock::now().time_since_epoch())
                          .count();
    if (maxFramesToSkip) {
    ...
    if (activeframes[lastfound] != 0 &&
        (Check_Sprites(frame, lastfound, nosprite, &nspr, frx, fry, spx, spy,
                       wid, hei) ||
         (*frameID >= 0))) {
      ...
      UINT ti = 0;
      while (ti < nspr) {
        Colorize_Sprite(frame, nosprite[ti], frx[ti], fry[ti], spx[ti], spy[ti],
                        wid[ti], hei[ti]);
      ...
      for (UINT ti = 0; ti < MAX_COLOR_ROTATIONS * 3; ti++) {
        lastrotations[ti] = rotations[ti] =
            colorrotations[lastfound * 3 * MAX_COLOR_ROTATIONS + ti];
      }
  ...
  UINT32 now = std::chrono::duration_cast<std::chrono::milliseconds>(
                   std::chrono::system_clock::now().time_since_epoch())
                   .count();
...
SERUM_API bool Serum_Colorize(UINT8* frame, int width, int height,
                              UINT8* palette, UINT8* rotations,
                              UINT32* triggerID) {
  UINT32 hashcode;
  int32_t frameID;
  return Serum_ColorizeWithMetadata(frame, width, height, palette, rotations,
...
SERUM_API bool Serum_ApplyRotations(UINT8* palette, UINT8* rotations) {
  ...
  for (int ti = 0; ti < MAX_COLOR_ROTATIONS; ti++) {
    ...
    if (elapsed >= (long long)(rotations[ti * 3 + 2] * 10)) {
      ...
      isrotation = true;
      UINT8 palsave[3 * 64];
      memcpy(palsave, &palette[rotations[ti * 3] * 3],
             (size_t)rotations[ti * 3 + 1] * 3);
...
SERUM_API bool Serum_ColorizeWithMetadataOrApplyRotations(
    ...
    UINT32* triggerID, UINT32* hashcode, int32_t* frameID) {
  bool new_frame = Serum_ColorizeWithMetadata(
      frame, width, height, palette, rotations, triggerID, hashcode, frameID);
  if (!new_frame) {
...
SERUM_API bool Serum_ColorizeOrApplyRotations(UINT8* frame, int width,
  ...
  if (frame) {
    ...
    UINT32 hashcode;
    int frameID;
    bool new_frame = Serum_ColorizeWithMetadataOrApplyRotations(
        frame, width, height, palette, rotations, triggerID, &hashcode,
        &frameID);
    if (new_frame) {
